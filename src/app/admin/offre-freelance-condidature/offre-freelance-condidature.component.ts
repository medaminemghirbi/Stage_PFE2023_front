...
@Component({
  selector: 'app-offre-freelance-condidature',
  templateUrl: './offre-freelance-condidature.component.html',
  styleUrls: ['./offre-freelance-condidature.component.css']
})
export class OffreFreelanceCondidatureComponent {
  minDate!: string;
  p:number = 1 ;
  counter:any
  dataArray:any = []
  ...
  ngOnInit(): void {
    ...
    this.adservice.getallcandidaturesfreelance(this.userdata.id).subscribe(data => {
      this.dataArray = data
  ...
  deleteCandFreelance(id:any){
    this.adservice.deleteCandFreelance(id).subscribe(
      () => {
